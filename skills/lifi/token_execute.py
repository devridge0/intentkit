...
class TokenExecute(LiFiBaseTool):
    ...
    def _format_quote_result(self, data: Dict[str, Any]) -> str:
        ...
        return f"Transfer from {from_token} to {to_token}, estimated amount: {to_amount}"
        ...
        **kwargs
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                from_address = wallet.addresses[0].address_id if wallet.addresses else ""
                ...
                if response.status_code != 200:
                    ...
                    return f"Error getting quote: {response.status_code} - {response.text}"
                ...
                if (from_token_address != "0x0000000000000000000000000000000000000000" and 
                    from_token_address != "" and 
                    approval_address):
                    try:
...
"""
    ...
    async def _check_and_set_allowance(
        ...
        try:
            ...
            allowance_data = self._encode_allowance_data(owner_address, approval_address)
            ...
            # Parse the result
            current_allowance = int(allowance_result, 16) if allowance_result.startswith('0x') else int(allowance_result)
            ...
            if current_allowance < amount_int:
                ...
                approve_data = self._encode_approve_data(approval_address, str(max_approval_amount))
    ...
    def _encode_allowance_data(self, owner_address: str, spender_address: str) -> str:
        ...
        owner_param = owner_address[2:].lower().zfill(64) if owner_address.startswith('0x') else owner_address.lower().zfill(64)
        spender_param = spender_address[2:].lower().zfill(64) if spender_address.startswith('0x') else spender_address.lower().zfill(64)
    ...
    async def _execute_transaction(self, wallet, transaction_request: Dict[str, Any], chain_id: str) -> str:
        ...
        try:
            ...
            try:
                ...
                raise Exception(f"Error sending transaction through CDP wallet: {str(e)}")
