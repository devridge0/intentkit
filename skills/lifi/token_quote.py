    ...
    def _format_quote_result(self, data: Dict[str, Any]) -> str:
        ...
        if gas_costs:
            ...
            for gas in gas_costs:
                ...
                if gas_amount_usd:
                    result += f"- {gas_amount_formatted} {gas_token} (${gas_amount_usd})\n"
                else:
        ...
        if data.get("includedSteps"):
            ...
            for i, step in enumerate(data.get("includedSteps", [])):
                tool = step.get("toolDetails", {}).get("name", step.get("tool", "Unknown"))
                ...
                if i == 0:
                    ...
                    result += f"{i+1}. {tool}: {from_token_symbol} → {to_token_symbol}\n"
