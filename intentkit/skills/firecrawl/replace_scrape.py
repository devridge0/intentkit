...
class FirecrawlReplaceScrapeInput(BaseModel):
    """Input for Firecrawl replace scrape tool."""
...
class FirecrawlReplaceScrape(FirecrawlBaseTool):
    ...
    name: str = "firecrawl_replace_scrape"
    description: str = (
    ...
    args_schema: Type[BaseModel] = FirecrawlReplaceScrapeInput
    ...
    async def _arun(
        ...
        **kwargs,
        ...
        """Implementation of the Firecrawl replace scrape tool.
        ...
        logger.debug(f"firecrawl_replace_scrape: Running scrape with context {context}")
        ...
        try:
            async with httpx.AsyncClient(timeout=timeout / 1000 + 10) as client:
                ...
                if response.status_code != 200:
                    logger.error(
                        f"firecrawl_replace_scrape: Error from Firecrawl API: {response.status_code} - {response.text}"
                    )
                    return (
                    ...
                    try:
                        ...
                        document = Document(
                            ...
                            metadata={
                                ...
                                "source_type": "firecrawl_replace_scrape",
                        ...
                        if agent_id:
                            ...
                            if existing_vector_store:
                                ...
                                try:
                                    ...
                                    logger.info(
                                        f"firecrawl_replace_scrape: Preserving {len(preserved_docs)} docs from other URLs, "
                            ...
                            if existing_metadata and existing_metadata.get("urls"):
                                ...
                                updated_metadata = metadata_manager.create_url_metadata(
                                    [url], [document], "firecrawl_replace_scrape"
                                )
                            ...
                            logger.info(
                                f"firecrawl_replace_scrape: Successfully replaced content for {url} with {len(split_docs)} chunks"
                            )
                        else:
                            formatted_result += "\n## Content Indexing\n"
                        ...
                        logger.error(
                            f"firecrawl_replace_scrape: Error indexing content: {index_error}"
                        )
                        formatted_result += "\n## Content Indexing\n"
            ...
            logger.error(f"firecrawl_replace_scrape: Timeout scraping URL: {url}")
            return (
                f"Timeout error: The request to scrape {url} took too long to complete."
            )
        except Exception as e:
            logger.error(
                f"firecrawl_replace_scrape: Error scraping URL: {e}", exc_info=True
            )
